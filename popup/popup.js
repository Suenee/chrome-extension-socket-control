...
async function refresh(){ const r=await chrome.runtime.sendMessage({type:"getStatus"}); lastStatus=r.status||{}; if(!loaded){apply(r.settings);loaded=true;} $("light").className=lastStatus.level||"gray"; $("state").textContent=label(lastStatus.state); $("detail").textContent=lastStatus.detail||""; $("toggle").textContent=isLive(lastStatus.state)?"Disconnect":"Connect"; $("diag").textContent=JSON.stringify({browser:navigator.userAgent,extension:"0.14",vpp:1,status:lastStatus,browserState:r.browserState},null,2); }
