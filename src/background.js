...
const DEFAULTS={host:"127.0.0.1",port:8170,socketBox:"chrome",apiKey:"",autoConnect:true};
...
async function initialize(){
  ...
  await setState(settings.autoConnect?"connecting":"disabled","Starting");
...
function connect(){
  ...
  const key=settings.apiKey ? "?apiKey="+encodeURIComponent(settings.apiKey) : "";
  ...
  ws.onclose=e=>{
    ...
    if(settings.autoConnect){setState("connecting","Reconnecting to SUB"); reconnectTimer=setTimeout(connect,3000);}
    else setState("disabled","Disconnected");
  };
}
...
function send(m){
  ...
  ws.send(JSON.stringify(m)); log("DEBUG","TX",m.type+" "+(m.method||m.event||""),{id:m.id});
}
async function receive(raw){
  ...
  if(m.correlationId && pending.has(m.correlationId)){
    const p=pending.get(m.correlationId); pending.delete(m.correlationId); clearTimeout(p.t);
    m.type==="error"?p.reject(new Error(m.error?.message||m.message||"VPP error")):p.resolve(m.result); return;
  }
  if(m.type==="call"){
    try {
      const result=await dispatch(m);
      if(m.expectsResponse) send(envelope(settings.socketBox,"response",{recipient:m.from,correlationId:m.id,result}));
      schedulePublish();
    } catch(e) {
      if(m.expectsResponse) send(envelope(settings.socketBox,"error",{recipient:m.from,correlationId:m.id,error:{code:e.code||"CHROME_API_ERROR",message:e.message||String(e)}}));
    }
  }
}
async function ping(){
...
async function publishState(){
  if(!admitted||!peerConnected||ws?.readyState!==WebSocket.OPEN)return;
  try{ const args=await getBrowserState(); send(envelope(settings.socketBox,"event",{event:"browserStateChanged",args,expectsResponse:false})); }
  catch(e){log("ERROR","STATE","Publish failed",String(e));}
}
async function setState(state,detail){
  const level=state==="connected"?"green":(state==="waiting"||state==="connecting")?"yellow":state==="disabled"?"gray":"red";
  await chrome.storage.local.set({connectionStatus:{state,level,detail,admitted,peerConnected,updatedAt:new Date().toISOString()}});
  try{await chrome.action.setBadgeText({text:level==="green"?"OK":level==="yellow"?"…":level==="red"?"!":""});}catch{}
}
function fail(e){ log("ERROR","SOCKET","Connect failed",String(e)); setState("error",e.message||String(e)); reconnectTimer=setTimeout(connect,3000); }
async function handleUi(m){
  ...
  if(m?.type==="saveSettings"){ settings={...DEFAULTS,...m.settings}; await chrome.storage.sync.set(settings); return {ok:true}; }
  if(m?.type==="connect"){ settings.autoConnect=true; await chrome.storage.sync.set({autoConnect:true}); connect(); return {ok:true}; }
  if(m?.type==="disconnect"){ disconnect(true); await chrome.storage.sync.set({autoConnect:false}); await setState("disabled","Disconnected by user"); return {ok:true}; }
  if(m?.type==="test"){ if(ws?.readyState!==WebSocket.OPEN) connect(); else await ping(); return {ok:true}; }
  return {ok:false};
}
