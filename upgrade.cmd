...
if not exist ".git" (
  ...
  git.exe init >NUL 2>&1
  if errorlevel 1 (
    popd
    exit /b 8
  )

  git.exe remote add origin https://github.com/Suenee/chrome-extension-socket-control.git >NUL 2>&1
)

git.exe remote set-url origin https://github.com/Suenee/chrome-extension-socket-control.git >NUL 2>&1
...
git.exe show origin/main:upgrade.ps1 > "%TMP_RUNNER%"
if errorlevel 1 (
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%TMP_RUNNER%" -RepositoryPath "%REPO_DIR%" -ActivePath "%ACTIVE_DIR%"
set "RC=%ERRORLEVEL%"
